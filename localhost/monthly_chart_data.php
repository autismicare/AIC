...
<?php
    ...
    $query ="SELECT User_ID, round(avg(Rating_ID), 2) as Average_Rating, DATE(Tracking_date) as Tracking_Date from MBTracker where User_ID = '$uid' and Tracking_date > DATE_ADD((select max(Tracking_date) from MBTracker where User_ID = '$uid'), INTERVAL -30 DAY)

...
<?php 
$get = "SELECT distinct(date(Tracking_date)) as TrackingDate from MBTracker where User_ID = '$uid' and Tracking_date > DATE_ADD((select max(Tracking_date) from MBTracker where User_ID = '$uid'), INTERVAL -700 DAY) order by TrackingDate";
...
<head>
    ...
    <script type="text/javascript" src="js/custom_chart.js"></script>
    <script>
...
<!-- Start Chart menu -->
    ...
    <a href="custom_show_chart.php" class="btn btn-info ml-2"> Custom Date Chart </a>
...
<!-- Start Table menu -->    
    <div class="d-flex justify-content-center">
    <form method="post" action="custom_chart_data.php">
...
<!-- End Table menu -->
    ...
    if (isset($_REQUEST['td'])) {
      ...
      (select * from MBTracker where User_ID = '$uid' and Tracking_date > DATE_ADD((select max(Tracking_date) from MBTracker where User_ID = '$uid'), INTERVAL -700 DAY)) as tr
